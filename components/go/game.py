from components.go.board import Board
from components.go.utils import Stone, make_2d_array
from components.go.group import GroupManager
from components.go.exceptions import (
...
class GameUI(object):
    ...
    def play_test(self):
        ...
        until the game is over.
        '''
        #self._place_stone(self._parse_move("E", "4"))
        #self._place_stone(Stone.WHITE, 4, 4)
        test = self._parse_coordinates("E 4")
        test2 = self._parse_coordinates("F 5")
        print(test)
        self._place_stone(test)
        self._switch_turns()
        self._place_stone(test2)
        while not self.game.is_over():
            is_turn_over = False
            self.game.render_board()
